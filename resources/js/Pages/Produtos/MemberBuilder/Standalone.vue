...
import {
    LayoutDashboard, Settings, Mail, Package, ArrowUpDown,
    ShoppingCart, Link2, Handshake, LayoutGrid, RotateCcw,
} from 'lucide-vue-next';
import MemberBuilderPreview from '@/components/member-builder/MemberBuilderPreview.vue';
import QuizReportModal from '@/components/produtos/QuizReportModal.vue';
...
const PRODUCT_TABS = [
    ...
    { id: 'mb_divider',      label: 'Área de Membros',   isDivider: true },
    { id: 'mb_modulos',      label: 'Módulos',           icon: Layers,        builderTab: 'modulos' },
    { id: 'mb_turmas',       label: 'Turmas',            icon: Users,         builderTab: 'turmas' },
    { id: 'mb_progresso',    label: 'Progresso',         icon: BarChart3,     builderTab: 'progresso' },
    { id: 'mb_comentarios',  label: 'Comentários',       icon: MessageSquare, builderTab: 'comentarios' },
    { id: 'mb_certificado',  label: 'Certificado',       icon: Award,         builderTab: 'certificado' },
...
const GAMIFICATION_TRIGGERS = [
    ...
    { value: 'completion_percent', label: 'X% da trilha' },
    { value: 'course_complete', label: 'Trilha completa (100%)' },
...
const quizReportOpen = ref(false);
const quizReportLesson = ref(null);
...
function openModulosLessonForm(lesson) {
    if (lesson) {
        ...
        modulosLessonForm.value = {
            ...
            quiz_questions: (Array.isArray(lesson.content_files?.questions) ? lesson.content_files.questions : []).map(q => ({
                type: 'scale',
                options: [],
                image_url: '',
                video_url: '',
...
function lessonPayload(form) {
    ...
    return {
        ...
        content_url: '',
        link_title: '',
        content_files: { questions: (form.quiz_questions ?? []) },
        release_after_days,
        release_at_date,
        content_text: '',
        duration_seconds: 0,
...
const editingModuleIsFree = ref(false);
...
function openModuleEdit(mod) {
    ...
    editingModuleIsFree.value = mod.is_free ?? false;
...
async function saveModuleTitle() {
    ...
    if (sectionType === 'courses') {
        ...
        payload.is_free = editingModuleIsFree.value;
...
function sectionTypeLabel(sectionType) {
    const map = { courses: 'Trilhas/Aulas', products: 'Outros produtos', external_links: 'Links externos' };
...
<template>
    <!-- Layout de 2 colunas: sidebar de abas do produto (esq) + conteúdo member builder (dir) -->
    <div class="flex flex-col lg:flex-row lg:gap-6 lg:pl-2">

        <!-- Sidebar de abas do produto (desktop) -->
        <aside class="hidden lg:flex lg:flex-col w-56 shrink-0 rounded-xl bg-zinc-50 dark:bg-zinc-800/50 p-2 self-start">
            <nav class="flex flex-col gap-0.5">
                <template v-for="tab in PRODUCT_TABS" :key="tab.id">
                    <div
                        v-if="tab.isDivider"
                        class="mt-3 mb-1 px-3 text-[10px] font-semibold uppercase tracking-widest text-zinc-400 dark:text-zinc-500"
                    >{{ tab.label }}</div>
                    <button
                        v-else-if="tab.builderTab"
                        type="button"
                        :class="[
                            'flex w-full items-center gap-2.5 rounded-lg px-3 py-2.5 text-left text-sm font-medium transition-all duration-200',
                            activeTab === tab.builderTab
                        ...
                        @click="activeTab = tab.builderTab"
                    >
                    ...
                    <a
                        v-else
                        :href="`/produtos/${produto.id}/edit?tab=${tab.id}`"
                        class="flex w-full items-center gap-2.5 rounded-lg px-3 py-2.5 text-left text-sm font-medium text-zinc-600 hover:bg-zinc-200/80 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-zinc-700/80 dark:hover:text-white transition-all duration-200"
                    >
                        <component :is="tab.icon" class="h-4 w-4 shrink-0" aria-hidden="true" />
                        {{ tab.label }}
                    </a>
        ...
        <!-- Abas mobile do produto (carrossel horizontal) -->
        <nav class="flex gap-2 overflow-x-auto pb-2 snap-x snap-mandatory no-scrollbar lg:hidden rounded-xl bg-zinc-100/80 p-1 dark:bg-zinc-800/80 mb-4">
            <template v-for="tab in PRODUCT_TABS" :key="tab.id">
                <span v-if="tab.isDivider" class="hidden" />
                <button
                    v-else-if="tab.builderTab"
                    type="button"
                    :class="[
                ...
                <a
                    v-else
                    :href="`/produtos/${produto.id}/edit?tab=${tab.id}`"
                    class="flex shrink-0 snap-center items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white transition-all duration-200"
                >
                    <component :is="tab.icon" class="h-4 w-4 shrink-0" />
                    {{ tab.label }}
                </a>
        ...
        <!-- Conteúdo member builder -->
        <div class="flex-1 min-w-0 flex flex-col rounded-xl overflow-hidden border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">

<!-- Conteúdo: sidebar config + preview (preview só em lg+) -->
        <div :class="['flex flex-col overflow-hidden', showPreview ? 'lg:flex-row' : '']" style="min-height: 70vh;">
            <aside
                :class="[
            ...
            >
                ...
                <div class="min-w-0 p-4">
                    ...
                    <template v-else-if="activeTab === 'modulos'">
                        ...
                        <h2 class="mb-1 text-sm font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Estrutura da trilha</h2>
                                                    ...
                                                    <button type="button" class="flex min-w-0 flex-1 flex-col items-stretch text-left" @click="selectModuleForAulas(mod.id)">
                                                        ...
                                                        <div class="min-w-0 flex-1 p-1.5">
                                                            ...
                                                            <div class="flex items-center gap-1 mt-0.5">
                                                                <span v-if="mod.is_free" class="rounded px-1 py-0.5 text-[9px] font-semibold bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">GRÁTIS</span>
                                            ...
                                            <template v-for="mod in section.modules" :key="'edit-' + mod.id">
                                                <div v-if="editingModuleId === mod.id" class="mt-3 rounded-xl border border-zinc-200 bg-zinc-50/80 p-3 dark:border-zinc-600 dark:bg-zinc-800/50">
                                                    ...
                                                    <div class="mb-3 flex items-center gap-3">
                                                        <Toggle
                                                            :model-value="editingModuleIsFree"
                                                            label="Módulo gratuito"
                                                            class="!mb-0"
                                                            @update:model-value="(v) => { editingModuleIsFree = v; }"
                                                        />
                                                        <span class="text-xs text-zinc-500 dark:text-zinc-400">Acessível sem compra</span>
                                                    </div>
                                                    <div class="mb-3">
                                ...
                                <div class="flex min-h-0 min-w-0 flex-1 flex-col overflow-y-auto p-4">
                                        ...
                                        <ul class="space-y-1">
                                            ...
                                            >
                                                <span class="flex min-w-0 flex-1 items-center gap-2 truncate">
                                                    ...
                                                    <svg v-else-if="lesson.type === 'quiz'" class="h-4 w-4 shrink-0 text-violet-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                                ...
                                                <div class="flex items-center gap-1">
                                                    <button v-if="lesson.type === 'quiz'" type="button" class="shrink-0 rounded p-1 text-violet-600 hover:bg-violet-50 dark:hover:bg-violet-950/30" title="Ver relatório do quiz" @click.stop="quizReportLesson = lesson; quizReportOpen = true"><svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg></button>
                                    ...
                                    <template v-else>
                                        <div class="space-y-3">
                                            <div>
                                                <label class="mb-1 block text-xs font-medium text-zinc-600 dark:text-zinc-400">Título</label>
                                                <input v-model="modulosLessonForm.title" type="text" :class="inputClass" class="w-full" placeholder="Título do quiz" />
                                            </div>
                                            <div>
                                                <label class="mb-1 block text-xs font-medium text-zinc-600 dark:text-zinc-400">Liberação</label>
                                                <div class="grid gap-2">
                                                    ...
                                                    <input v-if="modulosLessonForm.release_mode === 'days'" v-model="modulosLessonForm.release_after_days" type="number" min="1" step="1" :class="inputClass" class="w-full" placeholder="Ex.: 7" />
                                                    <input v-else-if="modulosLessonForm.release_mode === 'date'" v-model="modulosLessonForm.release_at_date" type="date" :class="inputClass" class="w-full" />
                                                </div>
                                            </div>
                                            ...
                                            <div class="space-y-3">
                                                <div class="flex items-center justify-between">
                                                    <label class="text-xs font-medium text-zinc-600 dark:text-zinc-400">Perguntas ({{ modulosLessonForm.quiz_questions.length }})</label>
                                                    <button type="button"
                                                        class="flex items-center gap-1 rounded-lg bg-emerald-600 px-2.5 py-1 text-xs font-medium text-white hover:bg-emerald-700"
                                                        @click="modulosLessonForm.quiz_questions.push({ id: Date.now().toString(36) + Math.random().toString(36).slice(2), type: 'scale', text: '', scale_min: 1, scale_max: 5, scale_min_label: '', scale_max_label: '', options: [], image_url: '', video_url: '', button_label: '', button_url: '', comment_enabled: false })">
                                                        + Pergunta
                                                    </button>
                                                </div>

                                                <div v-if="!modulosLessonForm.quiz_questions.length" class="rounded-lg border border-dashed border-zinc-300 p-4 text-center text-xs text-zinc-400 dark:border-zinc-600">
                                                    Nenhuma pergunta. Clique em "+ Pergunta" para adicionar.
                                                </div>

                                                <div v-for="(q, qi) in modulosLessonForm.quiz_questions" :key="q.id"
                                                    class="rounded-lg border border-zinc-200 bg-zinc-50 p-3 space-y-3 dark:border-zinc-700 dark:bg-zinc-800/50">

                                                    <!-- Cabeçalho da pergunta -->
                                                    <div class="flex items-center justify-between gap-2">
                                                        <span class="text-xs font-semibold text-zinc-500 dark:text-zinc-400">Pergunta {{ qi + 1 }}</span>
                                                        <div class="flex items-center gap-2">
                                                            <button v-if="qi > 0" type="button" class="text-xs text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200"
                                                                @click="modulosLessonForm.quiz_questions.splice(qi - 1, 0, modulosLessonForm.quiz_questions.splice(qi, 1)[0])">↑</button>
                                                            <button v-if="qi < modulosLessonForm.quiz_questions.length - 1" type="button" class="text-xs text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200"
                                                                @click="modulosLessonForm.quiz_questions.splice(qi + 1, 0, modulosLessonForm.quiz_questions.splice(qi, 1)[0])">↓</button>
                                                            <button type="button" class="text-xs text-red-500 hover:text-red-700" @click="modulosLessonForm.quiz_questions.splice(qi, 1)">Remover</button>
                                                        </div>
                                                    </div>

                                                    <!-- Tipo -->
                                                    <div>
                                                        <label class="mb-1 block text-xs text-zinc-500 dark:text-zinc-400">Tipo</label>
                                                        <select v-model="q.type" :class="inputClass" class="w-full"
                                                            @change="q.options = q.options?.length ? q.options : []">
                                                            <option value="scale">Escala numérica (1 a X)</option>
                                                    ...
                                                    <!-- Texto -->
                                                    <div>
                                                        <label class="mb-1 block text-xs text-zinc-500 dark:text-zinc-400">Enunciado</label>
                                                        <textarea v-model="q.text" :class="inputClass" class="w-full" rows="2" placeholder="Texto da pergunta..."/>
                                                    </div>
                                                    ...
                                                    <template v-if="q.type === 'scale'">
                                                        <div class="flex gap-2">
                                                            <div class="flex-1">
                                                                <label class="mb-1 block text-xs text-zinc-500 dark:text-zinc-400">Mínimo</label>
                                                                <select v-model.number="q.scale_min" :class="inputClass" class="w-full">
                                                                    <option :value="0">0</option>
                                                                    <option :value="1">1</option>
                                                                </select>
                                                            </div>
                                                            <div class="flex-1">
                                                                <label class="mb-1 block text-xs text-zinc-500 dark:text-zinc-400">Máximo</label>
                                                                <select v-model.number="q.scale_max" :class="inputClass" class="w-full">
                                                        ...
                                                        <div class="flex gap-2">
                                                            <div class="flex-1">
                                                                <label class="mb-1 block text-xs text-zinc-500 dark:text-zinc-400">Label mínimo (opcional)</label>
                                                                <input v-model="q.scale_min_label" type="text" :class="inputClass" class="w-full" placeholder="Ex: Discordo"/>
                                                            </div>
                                                            <div class="flex-1">
                                                                <label class="mb-1 block text-xs text-zinc-500 dark:text-zinc-400">Label máximo (opcional)</label>
                                                                <input v-model="q.scale_max_label" type="text" :class="inputClass" class="w-full" placeholder="Ex: Concordo"/>
                                                            </div>
                                                        </div>
                                                        <label class="flex items-center gap-2 text-xs text-zinc-600 dark:text-zinc-400 cursor-pointer">
                                                            <input type="checkbox" v-model="q.comment_enabled" class="rounded"/>
                                                            Permitir comentário opcional
                                                        </label>
                                                    </template>

                                                    <!-- Escolha única ou Múltipla -->
                                                    <template v-else-if="q.type === 'single' || q.type === 'multi'">
                                                        <div class="space-y-2">
                                                            <div class="flex items-center justify-between">
                                                                <label class="text-xs text-zinc-500 dark:text-zinc-400">
                                                                    Opções {{ q.type === 'multi' ? '(múltipla)' : '(única)' }}
                                                                </label>
                                                                <button type="button" class="text-xs text-emerald-600 hover:text-emerald-700 dark:text-emerald-400"
                                                                    @click="q.options.push({ id: Date.now().toString(36) + Math.random().toString(36).slice(2), text: '' })">
                                                                    + Opção
                                                                </button>
                                                            </div>
                                                            <div v-if="!q.options?.length" class="text-xs text-zinc-400 dark:text-zinc-500">Adicione ao menos duas opções.</div>
                                                            <div v-for="(opt, oi) in q.options" :key="opt.id" class="flex items-center gap-2">
                                                                <input v-model="opt.text" type="text" :class="inputClass" class="min-w-0 flex-1" :placeholder="`Opção ${oi + 1}`"/>
                                                                <button type="button" class="shrink-0 text-xs text-red-500 hover:text-red-700" @click="q.options.splice(oi, 1)">✕</button>
                                                            </div>
                                                        </div>
                                                    </template>

                                                    <!-- Mídia da pergunta -->
                                                    <div class="space-y-2 rounded-md border border-zinc-200 bg-white p-2.5 dark:border-zinc-700 dark:bg-zinc-900/30">
                                                        <p class="text-xs font-medium text-zinc-500 dark:text-zinc-400">Mídia (opcional)</p>
                                                        <div>
                                                            <label class="mb-1 block text-xs text-zinc-500 dark:text-zinc-400">URL do vídeo YouTube</label>
                                                            <input v-model="q.video_url" type="url" :class="inputClass" class="w-full" placeholder="https://youtube.com/watch?v=..."/>
                                                        </div>
                                                        <div>
                                                            <label class="mb-1 block text-xs text-zinc-500 dark:text-zinc-400">URL de imagem (se não usar vídeo)</label>
                                                            <input v-model="q.image_url" type="url" :class="inputClass" class="w-full" placeholder="https://..."/>
                                                        </div>
                                                    </div>

                                                    <!-- Botão da pergunta -->
                                                    <div class="space-y-2 rounded-md border border-zinc-200 bg-white p-2.5 dark:border-zinc-700 dark:bg-zinc-900/30">
                                                        <p class="text-xs font-medium text-zinc-500 dark:text-zinc-400">Botão de link (opcional)</p>
                                                        <div class="flex gap-2">
                                                            <div class="flex-1">
                                                                <label class="mb-1 block text-xs text-zinc-500 dark:text-zinc-400">Texto do botão</label>
                                                                <input v-model="q.button_label" type="text" :class="inputClass" class="w-full" placeholder="Ex: Saiba mais"/>
                                                            </div>
                                                            <div class="flex-1">
                                                                <label class="mb-1 block text-xs text-zinc-500 dark:text-zinc-400">URL de destino</label>
                                                                <input v-model="q.button_url" type="url" :class="inputClass" class="w-full" placeholder="https://..."/>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                    ...
                    <template v-else-if="activeTab === 'progresso'">
                        <div class="space-y-6">
                            ...
                            <div class="flex flex-wrap gap-4 rounded-xl border border-zinc-200 bg-zinc-50/50 px-4 py-3 text-sm dark:border-zinc-600 dark:bg-zinc-800/30">
                                ...
                                <div>
                                    <span class="text-zinc-500 dark:text-zinc-400">Aulas na trilha</span>
                    ...
                    <template v-else-if="activeTab === 'certificado'">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label class="mb-1 block text-xs font-medium text-zinc-600 dark:text-zinc-400">Duração da trilha</label>
    ...
    <QuizReportModal
        ...
        :lesson="quizReportLesson"
        @close="quizReportOpen = false"
    />
</template>
