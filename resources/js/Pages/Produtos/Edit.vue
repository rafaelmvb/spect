...
const BASE_TABS = [
    ...
    { id: 'instrucoes_ia', label: 'Instruções IA', icon: Sparkles },
    { id: 'mb_divider', label: 'Área de Membros', isDivider: true, showForType: 'area_membros' },
    { id: 'mb_modulos', label: 'Módulos', icon: Layers, linkOnly: true, showForType: 'area_membros', linkHref: (p) => `/produtos/${p.id}/member-builder?tab=modulos` },
    { id: 'mb_turmas', label: 'Turmas', icon: Users, linkOnly: true, showForType: 'area_membros', linkHref: (p) => `/produtos/${p.id}/member-builder?tab=turmas` },
    { id: 'mb_progresso', label: 'Progresso', icon: BarChart3, linkOnly: true, showForType: 'area_membros', linkHref: (p) => `/produtos/${p.id}/member-builder?tab=progresso` },
    { id: 'mb_comentarios', label: 'Comentários', icon: MessageSquare, linkOnly: true, showForType: 'area_membros', linkHref: (p) => `/produtos/${p.id}/member-builder?tab=comentarios` },
    { id: 'mb_certificado', label: 'Certificado', icon: Award, linkOnly: true, showForType: 'area_membros', linkHref: (p) => `/produtos/${p.id}/member-builder?tab=certificado` },
...
const props = defineProps({
    ...
    available_journeys: { type: Array, default: () => [] },
});
...
// ── Instruções IA ──────────────────────────────────────────────────────────────
const validJourneyIds = new Set(props.available_journeys.map(j => j.id));
const aiContext = reactive({
    instructions: props.produto.ai_context?.instructions ?? '',
    files: [...(props.produto.ai_context?.files ?? [])],
    jornadas: (props.produto.ai_context?.jornadas ?? []).filter(
        j => validJourneyIds.has(Number(j.journey_id))
    ),
});
...
function isJornadaSelected(journeyId) {
    return aiContext.jornadas.some((j) => j.journey_id === journeyId);
}

function toggleJornada(journeyId) {
    const idx = aiContext.jornadas.findIndex((j) => j.journey_id === journeyId);
    if (idx === -1) {
        aiContext.jornadas.push({ journey_id: journeyId, is_free: true });
    } else {
        aiContext.jornadas.splice(idx, 1);
    }
}

function getJornadaConfig(journeyId) {
    return aiContext.jornadas.find((j) => j.journey_id === journeyId);
}

function toggleJornadaFree(journeyId) {
    const cfg = getJornadaConfig(journeyId);
    if (cfg) cfg.is_free = !cfg.is_free;
}

async function saveAiInstructions() {
    aiSaving.value = true;
    try {
        await axios.put(`/produtos/${props.produto.id}/ai-context`, {
            instructions: aiContext.instructions,
            jornadas: aiContext.jornadas.map(j => ({ journey_id: Number(j.journey_id), is_free: Boolean(j.is_free) })),
        });
        aiSaved.value = true;
...
async function uploadAiFile(event) {
    ...
    try {
        const fd = new FormData();
        fd.append('file', file);
        const { data } = await axios.post(`/produtos/${props.produto.id}/ai-context/upload`, fd);
...
async function deleteAiFile(fileId) {
    await axios.delete(`/produtos/${props.produto.id}/ai-context/files/${fileId}`);
...
<template>
    <div class="flex flex-col lg:flex-row lg:gap-6 space-y-6 lg:space-y-0 lg:pl-2">
        ...
        >
            <nav class="flex flex-col gap-0.5">
                <template v-for="tab in TABS" :key="tab.id">
                    <div
                        v-if="tab.isDivider && (!tab.showForType || tab.showForType === produto.type)"
                        class="px-3 pt-3 pb-1 text-xs font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500"
                    >
                        {{ tab.label }}
                    </div>
                    <a
                        v-else-if="tab.linkOnly && (!tab.showForType || tab.showForType === produto.type)"
                        :href="tab.linkHref(produto)"
                        :class="[
        ...
        >
            <template v-for="tab in TABS" :key="tab.id">
                <a
                    v-if="tab.linkOnly && (!tab.showForType || tab.showForType === produto.type)"
                    :href="tab.linkHref(produto)"
                    :class="[
                            ...
                            <div class="space-y-5">
                                <div>
                                    ...
                                    <input
                                        ...
                                        type="text"
                                        required
                                        placeholder="Ex: Trilha Completa de X"
                                        :class="inputClass"
                                    />
        ...
        <!-- Aba Instruções IA -->
        <template v-if="currentTab === 'instrucoes_ia'">
            <div class="space-y-6">
                <!-- Instruções de texto -->
                <div class="overflow-hidden rounded-2xl border border-zinc-200/80 bg-white shadow-sm dark:border-zinc-700/80 dark:bg-zinc-800/95">
                    <div class="border-b border-zinc-200/80 px-6 py-4 dark:border-zinc-700/80">
                        ...
                        <p class="mt-0.5 text-sm text-zinc-500 dark:text-zinc-400">
                            Descreva o contexto, objetivos e características deste curso. A IA usará essas instruções ao gerar relatórios dos alunos.
                        </p>
                    </div>
                    <div class="p-6">
                        <textarea
                            v-model="aiContext.instructions"
                            rows="10"
                            placeholder="Ex.: Esta é uma trilha de neurociência aplicada ao TDAH. Os alunos são adultos diagnosticados ou em investigação. Ao gerar relatórios, priorize padrões de regulação emocional e foco executivo..."
                            class="w-full resize-y rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-900 placeholder-zinc-400 focus:border-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-500/20 dark:border-zinc-700 dark:bg-zinc-900 dark:text-white dark:placeholder-zinc-500"
                        />
                        <div class="mt-3 flex items-center justify-end gap-3">
                            <span v-if="aiSaved" class="flex items-center gap-1 text-sm text-green-600 dark:text-green-400">
                            ...
                            >
                                <Loader2 v-if="aiSaving" class="h-4 w-4 animate-spin" />
                ...
                <div class="overflow-hidden rounded-2xl border border-zinc-200/80 bg-white shadow-sm dark:border-zinc-700/80 dark:bg-zinc-800/95">
                    <div class="border-b border-zinc-200/80 px-6 py-4 dark:border-zinc-700/80">
                        ...
                        <p class="mt-0.5 text-sm text-zinc-500 dark:text-zinc-400">
                            PDFs, documentos e imagens que a IA poderá usar como contexto ao gerar relatórios deste curso.
                        </p>
                    </div>
                    <div class="p-6 space-y-4">
                        <!-- Zona de upload -->
                        <label class="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-zinc-300 bg-zinc-50 px-6 py-8 text-center transition hover:border-violet-400 hover:bg-violet-50 dark:border-zinc-700 dark:bg-zinc-900 dark:hover:border-violet-500 dark:hover:bg-zinc-800">
                            <input type="file" class="sr-only" accept=".pdf,.doc,.docx,.txt,.png,.jpg,.jpeg,.gif,.webp" @change="uploadAiFile" :disabled="aiUploading" />
                            <Upload v-if="!aiUploading" class="h-8 w-8 text-zinc-400 dark:text-zinc-500" />
                            <Loader2 v-else class="h-8 w-8 animate-spin text-violet-500" />
                        ...
                        <!-- Lista de arquivos -->
                        <div v-if="aiContext.files.length > 0" class="space-y-2">
                            <div
                                v-for="f in aiContext.files"
                                :key="f.id"
                                class="flex items-center gap-3 rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-3 dark:border-zinc-700 dark:bg-zinc-900"
                            >
                                <Image v-if="f.type === 'image'" class="h-5 w-5 flex-shrink-0 text-violet-400" />
                                <FileText v-else class="h-5 w-5 flex-shrink-0 text-zinc-400" />
                                <div class="min-w-0 flex-1">
                                ...
                                <a
                                    ...
                                    :href="f.url"
                                    target="_blank"
                                    class="flex-shrink-0 rounded p-1 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300"
                                    title="Ver arquivo"
                                >
                                    <Link2 class="h-4 w-4" />
                                </a>
                                <button
                                    ...
                                    class="flex-shrink-0 rounded p-1 text-zinc-400 hover:text-red-500 dark:hover:text-red-400"
                                    title="Remover"
                                >
                                    <Trash2 class="h-4 w-4" />
                                </button>
                ...
                <!-- Jornadas recomendadas pela IA -->
                <div class="overflow-hidden rounded-2xl border border-zinc-200/80 bg-white shadow-sm dark:border-zinc-700/80 dark:bg-zinc-800/95">
                    <div class="border-b border-zinc-200/80 px-6 py-4 dark:border-zinc-700/80">
                        <div class="flex items-center gap-2">
                            <Sparkles class="h-4 w-4 text-violet-500" />
                            <h2 class="text-base font-semibold text-zinc-900 dark:text-white">Jornadas Recomendadas pela IA</h2>
                        </div>
                        <p class="mt-0.5 text-sm text-zinc-500 dark:text-zinc-400">
                            Selecione quais jornadas a IA poderá liberar para o aluno após analisar o Mega Relatório. Define se cada uma é grátis ou paga.
                        </p>
                    </div>
                    <div class="p-6">
                        <div v-if="available_journeys.length === 0" class="rounded-lg border border-dashed border-zinc-300 px-4 py-8 text-center text-sm text-zinc-400 dark:border-zinc-700 dark:text-zinc-500">
                            Nenhuma jornada ativa encontrada. Crie jornadas no menu <strong>Jornadas</strong> primeiro.
                        </div>
                        <div v-else class="space-y-2">
                            <div
                                v-for="j in available_journeys"
                                :key="j.id"
                                class="flex items-center gap-4 rounded-lg border px-4 py-3 transition"
                                :class="isJornadaSelected(j.id)
                            ...
                            >
                                ...
                                <input
                                    type="checkbox"
                                    :checked="isJornadaSelected(j.id)"
                                    @change="toggleJornada(j.id)"
                                    class="h-4 w-4 cursor-pointer rounded accent-violet-500"
                                />

                                <!-- Nome e descrição -->
                                <div class="min-w-0 flex-1">
                                    <p class="text-sm font-medium text-zinc-800 dark:text-zinc-200">{{ j.name }}</p>
                                    <p v-if="j.description" class="truncate text-xs text-zinc-400 dark:text-zinc-500">{{ j.description }}</p>
                                </div>

                                <!-- Toggle Grátis / Pago -->
                                <div v-if="isJornadaSelected(j.id)" class="flex flex-shrink-0 items-center gap-1 rounded-full border border-zinc-300 bg-white p-0.5 text-xs dark:border-zinc-600 dark:bg-zinc-800">
                                    <button
                                        type="button"
                                        @click="() => { const c = getJornadaConfig(j.id); if(c) c.is_free = true; }"
                                        class="rounded-full px-3 py-1 font-medium transition"
                                        :class="getJornadaConfig(j.id)?.is_free
                                            ? 'bg-green-500 text-white shadow'
                                            : 'text-zinc-500 hover:text-zinc-700 dark:text-zinc-400'"
                                    >Grátis</button>
                                    <button
                                        type="button"
                                        @click="() => { const c = getJornadaConfig(j.id); if(c) c.is_free = false; }"
                                        class="rounded-full px-3 py-1 font-medium transition"
                                        :class="!getJornadaConfig(j.id)?.is_free
                                ...
                                <span v-else class="flex-shrink-0 text-xs text-zinc-400 dark:text-zinc-600">não selecionada</span>
                            </div>
                        </div>

                        <p class="mt-3 text-xs text-zinc-400 dark:text-zinc-500">
                            A liberação só ocorre automaticamente se as instruções de IA acima também estiverem preenchidas.
                        </p>
                        ...
                        <div class="mt-4 flex items-center justify-end gap-3">
                            ...
                            >
                                <Loader2 v-if="aiSaving" class="h-4 w-4 animate-spin" />
                                {{ aiSaving ? 'Salvando...' : 'Salvar jornadas' }}
                            </Button>
                        </div>
                    </div>
                </div>
