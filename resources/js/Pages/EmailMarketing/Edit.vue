...
const existingFilter = props.campaign.filter_config || {};
// Normaliza filtros antigos (all_customers/product_ids) para o novo formato (type)
const normalizedFilter = (() => {
    if (existingFilter.type) return existingFilter;
    if (existingFilter.product_ids?.length) return { type: 'bought_product', product_ids: existingFilter.product_ids };
    return { type: 'all_customers', product_ids: [] };
})();
...
const form = useForm({
    ...
    filter_config: { inactive_days: 30, max_progress: 50, min_progress: 80, date_from: '', date_to: '', ...normalizedFilter },
});
...
function useDefaultTemplate() { form.body_html = props.default_body_html || ''; }
    ...
    <div class="mx-auto max-w-4xl space-y-6">
        ...
        <form class="space-y-6 rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-800/50"
            ...
            <div class="flex flex-wrap items-center gap-3 border-t border-zinc-200 pt-4 dark:border-zinc-700">
                ...
                <Button v-if="email_configured" type="button" variant="outline" @click="confirmSend">
                    🚀 Disparar campanha
                </Button>
