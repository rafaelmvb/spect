...
import { PanelsTopLeft, Bell, CircleHelp } from 'lucide-vue-next';
...
import PageHelpModal from '@/components/layout/PageHelpModal.vue';
import { getPageHelp } from '@/data/pageHelp.js';
...
const currentHelp = computed(() => getPageHelp(page.url));
</script>
...
<template>
    <header class="z-[99998] flex shrink-0 w-full items-center justify-between gap-4 bg-transparent px-4 py-3 lg:px-6 lg:py-4">
        <div class="flex min-w-0 flex-1 items-center gap-3">
            ...
            <!-- Botão de ajuda: aparece em qualquer página que tenha conteúdo de ajuda -->
            <button
                v-if="currentHelp"
                type="button"
                class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-zinc-400 transition-colors hover:bg-[var(--color-primary)]/10 hover:text-[var(--color-primary)] dark:text-zinc-500 dark:hover:text-[var(--color-primary)]"
