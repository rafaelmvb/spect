...
class EmailMarketingController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        $validated = $request->validate([
            ...
            'filter_config.inactive_days' => ['nullable', 'integer', 'min:1'],
            'filter_config.max_progress' => ['nullable', 'numeric', 'min:0', 'max:100'],
            'filter_config.min_progress' => ['nullable', 'numeric', 'min:0', 'max:100'],
            'filter_config.date_from' => ['nullable', 'date'],
            'filter_config.date_to' => ['nullable', 'date'],
        ]);

        $tenantId = auth()->user()->tenant_id;
        $filterConfig = $validated['filter_config'] ?? [];
        if (empty($filterConfig['type'])) {
            $filterConfig['type'] = 'all_customers';
        }
