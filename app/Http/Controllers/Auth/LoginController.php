    ...
    public function login(Request $request): RedirectResponse
    {
        ...
        if (Auth::attempt($credentials, (bool) $request->boolean('remember'))) {
            ...
            if (! $user->canAccessPanel()) {
                ...
                $request->session()->invalidate();
                $request->session()->regenerateToken();
                return back()->withErrors(['email' => 'Esta página é exclusiva para administradores. Para acessar sua área de membros, utilize o link fornecido pelo seu instrutor.'])->onlyInput('email');
            }
            if ($user->isProfissional()) {
                return redirect()->intended('/p/dashboard');
            }
            if ($user->tenant_id) {
                TeamAuditLog::create([
