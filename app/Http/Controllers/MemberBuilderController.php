...
class MemberBuilderController extends Controller
{
    ...
    public function index(Product $produto): Response|RedirectResponse
    {
        ...
        return Inertia::render('Produtos/MemberBuilder/Standalone', [
    ...
    public function updateConfig(Request $request, Product $produto): JsonResponse|RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            // login_without_password removido — acesso somente via email + senha
    ...
    public function storeLesson(Request $request, Product $produto, MemberModule $module): JsonResponse|RedirectResponse
    {
        ...
        $validated = $request->validate([
            'title' => ['required', 'string', 'max:255'],
            'type' => ['required', 'string', 'in:video,link,pdf,pdf_presentation,pdf_reader,text,quiz'],
            'content_url' => ['nullable', 'string', 'max:2000'],
            'link_title' => ['nullable', 'string', 'max:255'],
            'content_files' => ['nullable'],
        ...
        $isQuiz = ($validated['type'] ?? null) === 'quiz';
        $contentFiles = $isQuiz ? null : $this->normalizeLessonContentFiles($request->input('content_files'));
        $quizData = $isQuiz ? $request->input('content_files') : null;
        if (in_array($validated['type'] ?? null, [MemberLesson::TYPE_PDF, MemberLesson::TYPE_PDF_PRESENTATION, MemberLesson::TYPE_PDF_READER], true)
            && empty($validated['content_url']) && is_array($contentFiles) && count($contentFiles) > 0) {
        ...
        MemberLesson::create([
            ...
            'content_files' => $isQuiz
                ? $quizData
                : (in_array($validated['type'], [MemberLesson::TYPE_PDF, MemberLesson::TYPE_PDF_PRESENTATION, MemberLesson::TYPE_PDF_READER], true)
    ...
    public function updateLesson(Request $request, Product $produto, MemberLesson $lesson): JsonResponse|RedirectResponse
    {
        ...
        $validated = $request->validate([
            'title' => ['sometimes', 'string', 'max:255'],
            'position' => ['sometimes', 'integer', 'min:0'],
            'type' => ['sometimes', 'string', 'in:video,link,pdf,pdf_presentation,pdf_reader,text,quiz'],
            'content_url' => ['nullable', 'string', 'max:2000'],
            'link_title' => ['nullable', 'string', 'max:255'],
            'content_files' => ['nullable'],
            'content_files.*.url' => ['nullable', 'string', 'url', 'max:2000'],
        ...
        $isQuiz = $type === 'quiz';
        if ($isQuiz) {
            $validated['content_files'] = $request->input('content_files');
        } else {
            ...
            if (in_array($type, [MemberLesson::TYPE_PDF, MemberLesson::TYPE_PDF_PRESENTATION, MemberLesson::TYPE_PDF_READER], true)) {
                if (is_array($contentFiles) && count($contentFiles) > 0) {
    ...
    public function storeNewAluno(Request $request, Product $produto): JsonResponse|RedirectResponse
    {
        ...
        ], User::ROLE_ALUNO, $produto->tenant_id);
    ...
    public function storeCommunityPage(Request $request, Product $produto): JsonResponse|RedirectResponse
    {
        ...
        if ($request->boolean('is_default')) {
            MemberCommunityPage::where('tenant_id', $produto->tenant_id)->update(['is_default' => false]);
        }
        $max = MemberCommunityPage::where('tenant_id', $produto->tenant_id)->max('position') ?? 0;
    ...
    {
        $this->authorizeProduct($produto);
        if ($page->tenant_id !== $produto->tenant_id) {
            abort(404);
        }
        $validated = $request->validate([
            'title' => ['sometimes', 'string', 'max:255'],
        ...
        if ($request->boolean('is_default')) {
            MemberCommunityPage::where('tenant_id', $produto->tenant_id)->where('id', '!=', $page->id)->update(['is_default' => false]);
        }
